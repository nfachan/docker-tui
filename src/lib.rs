...
impl App {
    ...
    fn main_loop(&mut self, docker: Docker, mut terminal: Terminal<impl Backend>) -> Result<()> {
        ...
        while let Some(event) = receiver.blocking_recv() {
            ...
            if let ControlFlow::Break(_) =
                self.handle_container_list_events(messages_out, &mut docker_sender)?
            {
                return Ok(());
            }
            terminal.draw(|frame| frame.render_widget(&mut container_list, frame.area()))?;
...
fn main_start_up(docker: Docker) -> Result<()> {
    ...
    App.main_loop(docker, Terminal::new(CrosstermBackend::new(stdout))?)
}
